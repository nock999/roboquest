      ...
      <div className="sticky top-0 z-30 bg-dark-950/95 backdrop-blur-sm border-b border-dark-800 px-4 py-3">
        <div className="flex items-center gap-3 max-w-2xl mx-auto">
          ...
          <ScreenIndicator
            ...
            className="flex-1"
          />
          <span className="text-xs text-dark-500 font-medium tabular-nums">
            {currentScreen + 1}/{lesson.screens.length}
          </span>
        </div>
      </div>
      ...
      <div className="sticky bottom-0 bg-dark-950/95 backdrop-blur-sm border-t border-dark-800 px-4 py-3">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-dark-400 hover:text-white hover:bg-dark-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all touch-target"
          >
          ...
          <button
            onClick={handleNext}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-semibold transition-all active:scale-95 touch-target"
          >
...
function ScreenRouter({
  ...
  switch (screen.type) {
    case 'hook':
      return <HookScreen screen={screen} />;
    case 'concept':
      return <ConceptScreen screen={screen} />;
    case 'interactive':
      return <InteractiveScreen screen={screen} onComplete={onComplete} />;
    case 'guided_challenge':
    case 'solo_challenge':
      return <ChallengeScreen screen={screen} onComplete={onComplete} />;
    case 'reflect':
      return <ReflectScreen screen={screen} onNext={onNext} onComplete={onComplete} />;
    default:
      return <div className="text-dark-400">Unknown screen type</div>;
  }
}
